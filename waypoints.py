...
def main():
    ...
    # Set the radius for Moon's orbit around Earth
    orbit_radius = 0.5  # in meters
    num_points_for_moon = len(x1)

    # Time intervals for drone movement
    ...
    with SyncCrazyflie(URI_EARTH, cf=Crazyflie(rw_cache='./cache')) as scf_earth, \
        ...
        start_position_logging(scf_earth, log_position_earth_callback)
        start_position_logging(scf_moon, log_position_moon_callback)
        ...
        try:
            for i in range(len(x1)):
                # Current positions for Earth and Moon
                ...
                # Check if Earth is within epsilon
                if reached_waypoint(current_position_earth, current_earth_position, epsilon) and \
                   within_z_leeway(current_position_earth[2], 1.5, z_epsilon):
                    earth.go_to(x1[(i+1) % len(x1)], y1[(i+1) % len(y1)], 1.5, 0, 1.0, relative=False)

                # Check if Moon is within epsilon
                if reached_waypoint(current_position_moon, current_moon_position, epsilon) and \
                   within_z_leeway(current_position_moon[2], 1.5, z_epsilon):
