...
def main():
    ...
    all_positions = [[compute_positions(t) for t in t_array] for _ in range(7)]
    ...
    # Write each point's path to its own CSV file
    for i, file_name in enumerate(file_names):
        with open(file_name, mode='w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['t', 'x', 'y', 'z', 'yaw'])  # CSV header
            for t, pos in zip(t_array, (point[i] for point in all_positions)):
                writer.writerow([t, pos[0], pos[1], pos[2], 0])  # Constant yaw
